  ...
  <UDashboardPanel id="notifications">
    <template #body>
      <div class="mb-5 flex items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap justify-end gap-2"><UBadge color="error" variant="soft">{{ unreadCount }} новых</UBadge><UButton v-if="pushSupported" color="primary" variant="outline" icon="i-lucide-smartphone" :loading="enablingPush" @click="enablePhoneNotifications">{{ pushPermission === 'granted' ? 'Перерегистрировать уведомления' : 'Включить уведомления на телефон' }}</UButton><UButton color="primary" variant="outline" icon="i-lucide-send" :loading="sendingTestPush" @click="sendTestNotification">Тестовое уведомление</UButton><UButton color="primary" icon="i-lucide-search-check" :loading="checking" @click="runTodayCheck">Проверить заказы сегодня</UButton><UButton color="neutral" variant="outline" icon="i-lucide-check-check" :disabled="!unreadCount" @click="markAllRead">Прочитать все</UButton><UButton color="neutral" variant="outline" icon="i-lucide-refresh-cw" @click="refresh()">Обновить</UButton></div>
      </div>
